 ...
 * ## 思路:
 * ### 方法一
 * 这道题是要将两个数组中相交的部分取出来,而且还得确保唯一性.在这种情况下我们就可以使用set集合来进行解答
 * 1. 先循环第一个数组,将其内的元素都放入一个临时的HashSet集合中去
 * 2. 第二部则是循环第二个数组,判断当前元素在临时HashSet集合中是否拥有,如果拥有的话,则将该元素放入答案HashSet集合中去
 * 3. 返回答案HashSet集合
 * ## 成绩:(方法一)
